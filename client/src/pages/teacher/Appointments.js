...
const TeacherAppointments = () => {
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:5000/api/v1/teacher/appointments/${id}`, {
        method: "PUT",
  ...
  const deleteAppointment = async (id) => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:5000/api/v1/user/delete-appointment/${id}`, {
        method: "DELETE",
